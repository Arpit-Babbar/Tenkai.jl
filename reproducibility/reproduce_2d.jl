...
sol_composite_blend_fo = trixi_include("run_rotate_composite.jl", limiter = :blend,
                                       blend_type = :FO,
                                       saveto = joinpath(composite_dir, "blend_fo"))
plot2d(sol_composite_blend_fo, "composite", "blend_FO")
sol_composite_blend_mh = trixi_include("run_rotate_composite.jl", limiter = :blend,
                                       blend_type = :MH,
...
let
    ...
    for limiter_name in (:blend,) #= :no_limiter,=#
        for degree in 3:4
            for i in 1:nsamples
                nx = grid_sizes[i]
                global sol_isentropic = trixi_include("run_isentropic.jl",
                                                      grid_size = [nx, nx],
                                                      degree = degree,
                ...
                writedlm(joinpath(dir, "l1_$(degree)_$(limiter_name).txt"),
                         zip(grid_sizes, l1_errors))
                writedlm(joinpath(dir, "l2_$(degree)_$(limiter_name).txt"),
                         zip(grid_sizes, l2_errors))
            end
        end
    end
end
...
let
    ...
    global sol_rp12 = trixi_include("run_$(test_case).jl", limiter = :blend,
                                    saveto = joinpath(joinpath(data_dir(), test_case),
                                                      "blend"))
...
let
    ...
    global sol_sedov_gassner = trixi_include("run_$(test_case).jl", limiter = :blend,
                                             saveto = joinpath(joinpath(data_dir(),
                                                                        test_case),
                                                               "blend"))
