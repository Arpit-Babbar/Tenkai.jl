...
try
    ...
    limiter_name = :tvb
    trixi_include("run_blast.jl", limiter = limiter_name,
                  saveto = joinpath(data_dir(), "blast_$limiter_name"))

    limiter_name = :blend
    test_case = "shuosher"
    for blend_order in (:FO, :MH)
        trixi_include("run_$(test_case).jl", limiter = limiter_name,
                      blend_type = blend_order,
                      saveto = joinpath(data_dir(),
                                        "$(test_case)_$(limiter_name)_$(blend_order)"))
    end
    limiter_name = :tvb
    trixi_include("run_$(test_case).jl", limiter = limiter_name,
                  saveto = joinpath(data_dir(), "$(test_case)_$limiter_name"))
