...
function plot_euler_python( test_case, title; plt_type = "sol",
   ...
   ax_density.set_xlabel("x"); ax_pressure.set_xlabel("x")
   ax_density.set_ylabel("Density"); ax_pressure.set_ylabel("Pressure")
   ax_density.grid(true, linestyle = "--"); ax_pressure.grid(true, linestyle = "--")

   # Set scales
   ax_density.set_xscale(xscale); ax_density.set_yscale(yscale)
   ax_pressure.set_xscale(xscale); ax_pressure.set_yscale(yscale)
   ...
   for i=1:n_plots
      ...
      if plt_type == "avg"
         @views ax_density.plot(soln_data[:,1], soln_data[:,2], markers[i], fillstyle = "none",
                                c = colors[i], label = label)
         @views ax_pressure.plot(soln_data[:,1], soln_data[:,4], markers[i], fillstyle = "none",
                                 c = colors[i], label = label)
      elseif plt_type in ("cts_sol","cts_avg")
         ...
         @views ax_density.plot(soln_data[1:nu,1], soln_data[1:nu,2], fillstyle = "none",
                                color = colors[i], label = label, linewidth = soln_line_width)
         @views ax_pressure.plot(soln_data[1:nu,1], soln_data[1:nu,4], fillstyle = "none",
                                 color = colors[i], label = label, linewidth = soln_line_width)
         for ix=2:nx
            i1 = (ix-1)*nu+1
            i2 = ix*nu
            @views ax_density.plot(soln_data[i1:i2,1], soln_data[i1:i2,2], fillstyle = "none",
                              c = colors[i], linewidth = soln_line_width)
            @views ax_pressure.plot(soln_data[i1:i2,1], soln_data[i1:i2,4], fillstyle = "none",
                              c = colors[i], linewidth = soln_line_width)
         end
      end
   end
   ax_density.legend(); ax_pressure.legend()
   ax_density.set_title(title); ax_pressure.set_title(title)
...
quick_plot!(
            p, u_x, u_y, seriestype, label; color_index, legend, markershape = :none,
...
function plot_euler_julia( test_case, title; plt_type = "cts_sol", legends = nothing, show_tvb = true,
                          degree = 4 # Degree to be manually specified by user
                          )
   plot_names = [test_case*"_blend_FO", test_case*"_blend_MH"]
   if show_tvb
      plot_names = [test_case*"_tvb", plot_names...]
      ...
      if plt_type == "avg"
         for n=1:nvar
            @views quick_plot!(p[n], soln_data[:,1], soln_data[:,n+1], seriestype,
                               label, color_index = i, legend = true, markerstrokestyle = :none,
                               markershape = markershapes[i], markersize = 2.5, linewidth = 2)
         end
      elseif plt_type in ("cts_sol","cts_avg")
         for n=1:nvar
            @views quick_plot!(p[n], soln_data[:,1], soln_data[:,n+1], seriestype,
         ...
         for n=1:nvar
            @views quick_plot!(p[n], soln_data[1:nu,1], soln_data[1:nu,n+1], seriestype,
                               markersize = 0.0,
                               label, markershape = :none, color_index = i, legend = true)
            for ix=2:nx
               i1 = (ix-1)*nu+1
               i2 = ix*nu
...
function add_theo_factors!(ax, ncells, error, degree, i,
   ...
   markers = ["s", "o", "*"]
   # if i == 1
      ax.loglog(xaxis,y,label="\$ O(M^{-$(d + 1)})\$",linestyle = "--", marker = markers[i], c = "grey",
                fillstyle = "none")
   # else
...
function plot_python_ncells_vs_y(; legend = nothing,
   ...
   for (i,degree) in enumerate(degrees)
      ...
      add_theo_factors!(ax_error, error[:,1], error, degree, i,
                        theo_factor_even, theo_factor_odd)
   end
...
function plot_python_ndofs_vs_y(; legend = nothing,
   ...
   for (i,degree) in enumerate(degrees)
      ...
      error = readdlm(joinpath(dir, "$(error_norm)_$(degree)_$(limiter).txt"))
      marker = markers[i]
      ax_error.loglog(error[:,1]*(d+1), error[:,2], marker = marker, c = colors[d], mec = "k",
                      fillstyle = "none", label = "\$ N = $d \$")
      add_theo_factors!(ax_error, (d+1)*error[:,1], error, degree, i,
                        theo_factor_even, theo_factor_odd)
   end
...
plot_python_ncells_vs_y(degrees = ["3","4"], saveto_dir = ".", log_sub = 2.5, error_norm = "l2", limiter = "blend", figsize = fig_size_())
plot_python_ncells_vs_y(degrees = ["3","4"], saveto_dir = ".", log_sub = 2.5, error_norm = "l2", limiter = "no_limiter", figsize = fig_size_())

plot_python_ndofs_vs_y(degrees = ["3","4"], saveto_dir = ".", log_sub = 2.5, error_norm = "l2", limiter = "blend", figsize = fig_size_())
plot_python_ndofs_vs_y(degrees = ["3","4"], saveto_dir = ".", log_sub = 2.5, error_norm = "l2", limiter = "no_limiter", figsize = fig_size_())