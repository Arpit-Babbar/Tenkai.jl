...
bound_limit = "no"
bflux = extrapolate
final_time = 10.0 # Choose 0.125 for sod, two_shock; 0.15 for two_rare_iv; 0.05 for two_rare_vacuum_iv
...
exact_shear = readdlm("exact_shear.txt", skipstart = 1)
plot!(p_ua[2], exact_shear[:, 1] .+ 0.5, exact_shear[:, 2], label = "Exact", lw = 2)
plot!(p_ua[3], exact_shear[:, 1] .+ 0.5, exact_shear[:, 3], label = "Exact", lw = 2)
plot!(p_ua[4], exact_shear[:, 1] .+ 0.5, exact_shear[:, 4], label = "Exact", lw = 2)
plot!(p_ua[5], exact_shear[:, 1] .+ 0.5, exact_shear[:, 5] .* exact_shear[:, 2],
      label = "Exact", lw = 2)
plot!(p_ua[6], exact_shear[:, 1] .+ 0.5, exact_shear[:, 6] .* exact_shear[:, 2],
      label = "Exact", lw = 2)
plot!(p_ua[7], exact_shear[:, 1] .+ 0.5, exact_shear[:, 7] .* exact_shear[:, 2],
      label = "Exact", lw = 2)

# ylims!(p_ua[2], (0.01, 0.02))
ylims!(p_ua[3], minimum(exact_shear[:, 3]) - 0.01, maximum(exact_shear[:, 3]) + 0.2)
ylims!(p_ua[4], -0.2, 0.2)
ylims!(p_ua[5], 0.0, 0.002)
