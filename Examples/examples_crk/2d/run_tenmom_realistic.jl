...
@inline function additional_energy_source(u, X, t, nuT, equations::Eq.TenMoment2D)
    x, y = X
    W_ = W(x, y)
    rho = u[1]
    E11 = nuT * rho * W_
    return SVector(0.0, 0.0, 0.0, E11, 0.0, 0.0)
end

nuT_ = 1.0
source_term = (u, x, t, equations::Eq.TenMoment2D) -> (additional_energy_source(u, x, t,
                                                                                nuT_,
                                                                                equations) +
...
param = Parameters(grid_size, cfl, bounds, save_iter_interval, save_time_interval,
                   ...
                   saveto = joinpath(@__DIR__,
                                     "output_N$(degree)_$(nx)_nuT$(nuT_)_$(limiter.name)"))
#------------------------------------------------------------------------------
