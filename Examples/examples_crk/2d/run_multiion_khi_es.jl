...
function flux_conservative_ramirez(u1, u2, i, eq)
    Trixi.flux_ruedaramirez_etal(u1, u2, i, eq.trixi_equations)
end

function flux_noncons_ramirez(u1, u2, i, eq)
    Trixi.flux_nonconservative_ruedaramirez_etal(u1, u2, i, eq.trixi_equations)
end
flux_conservative = flux_conservative_ramirez
flux_nonconservative = flux_noncons_ramirez

solver = cRK44()

volume_flux = (Trixi.flux_ruedaramirez_etal, Trixi.flux_nonconservative_ruedaramirez_etal)
volume_integral = Tenkai.VolumeIntegralFluxDifferencing(volume_flux)
solver = TrixiRKSolver(volume_integral)
# solver = cRK22(MyVolumeIntegralFluxDifferencing(degree,
#                                                 flux_conservative,
#                                                 flux_nonconservative))
