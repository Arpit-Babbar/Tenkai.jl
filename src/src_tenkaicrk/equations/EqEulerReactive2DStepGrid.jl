...
function compute_time_step(eq::EulerReactive2D, problem, grid::StepGrid, aux, op, cfl, u1,
                           ua)
    @timeit aux.timer "Time Step computation" begin
        ...
        @threaded for element_index in element_iterator(grid) # Loop over cells
            ...
            sx, sy = max_abs_eigen_value(eq, 1, u_node), max_abs_eigen_value(eq, 2, u_node)
...
# Limiters
#------------------------------------------------------------------------------
function apply_tvb_limiterβ!(eq::EulerReactive2D, problem, scheme, grid::StepGrid, param,
                             op, ua, u1, aux)
...
function apply_tvb_limiter!(eq::EulerReactive2D, problem, scheme, grid::StepGrid, param, op,
                            ua, u1, aux)
    @timeit aux.timer "TVB Limiter" begin
        nx, ny = grid.size
        @unpack xg, wg, Vl, Vr = op
...
function write_poly(eq::EulerReactive2D, grid::StepGrid, op, u1, fcount)
        ...
        for i in 1:nx_tuple[2]
            ...
            for jy in 1:nd
                ...
                for ix in 1:nd
                    ...
                    u_equi_pres[i_min + ix - 1, j_index] = pressure(eq, u_node)
