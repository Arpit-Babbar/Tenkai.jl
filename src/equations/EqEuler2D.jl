...
function Tenkai.write_soln!(base_name, fcount, iter, time, dt, eq::Euler2D,
    ...
    vtk["Density"] = Float64.(density_arr)
    vtk["Velocity_x"] = Float64.(velx_arr)
    vtk["Velocity_y"] = Float64.(vely_arr)
    vtk["Pressure"] = Float64.(pres_arr)
    for j in 1:ny, i in 1:nx
        @views con2prim!(eq, exact_data[i, j], prim[:, i, j])
    end
    @views vtk["Exact Density"] = Float64.(prim[1, 1:nx, 1:ny])
    @views vtk["Exact Velocity_x"] = Float64.(prim[2, 1:nx, 1:ny])
    @views vtk["Exact Velocity_y"] = Float64.(prim[3, 1:nx, 1:ny])
    @views vtk["Exact Pressure"] = Float64.(prim[4, 1:nx, 1:ny])
    ...
    element_variables[:density] = Float64.(vec(density_arr))
    element_variables[:velocity_x] = Float64.(vec(velx_arr))
    element_variables[:velocity_y] = Float64.(vec(vely_arr))
    element_variables[:pressure] = Float64.(vec(pres_arr))
...
function save_solution_file(u_, time, dt, iter,
    ...
    h5open(filename, "w") do file
        ...
        for v in 1:n_vars
            # Convert to 1D array
            file["variables_$v"] = Float64.(vec(data[v, .., :]))
        ...
        for (v, (key, element_variable)) in enumerate(element_variables)
            # Add to file
            file["element_variables_$v"] = Float64.(element_variable)
