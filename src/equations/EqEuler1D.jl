...
function prim2con(eq::Euler1D, prim) # primitive, gas constant
   @unpack γ = eq
   U = SVector(prim[1], prim[1]*prim[2], prim[3]/(γ-1.0) + 0.5*prim[1]*prim[2]^2)
...
lax_iv(x) = riemann_problem([0.445,0.698,3.528], [0.5,0.,0.571], 0., x)
...
function chandrashekar!(x, ual, uar, Fl, Fr, Ul, Ur, eq::Euler1D, dir, F)
   ...
   F[3] = F[1] * 0.5*(1/(eq.γ-1.0)/beta_mean - velocity_square_avg)+F[2]*v1_avg
   return nothing
end
...
function roe(x, ual, uar, Fl, Fr, Ul, Ur, eq::Euler1D, dir)
   ...
   F2 = 0.5*(Fl[2] + Fr[2]) - 0.5*( α1 * l1 * (u-c) + α2 * l2 * u
                                       + α3 * l3 * (u+c) )
   F3 = 0.5*(Fl[3] + Fr[3]) - 0.5*( α1 * l1 * (H - u*c) + 0.5 * α2 * l2 * u*u
                                       + α3 * l3 * (H + u*c) )
...
function eroe(x, ual, uar, Fl, Fr, Ul, Ur, eq::Euler1D, dir)
   ...
   l1, l2, l3 = abs(u-c), abs(u), abs(u+c)
   δ  = c*ϵ
   if abs(l1)<2.0*ϵ l1 = 0.5 * (l1^2/δ + δ) end # entropy correction
   if abs(l3)<2.0*ϵ l3 = 0.5 * (l3^2/δ + δ) end # entropy correction
