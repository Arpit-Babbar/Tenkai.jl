...
# Fill ghost values
#-------------------------------------------------------------------------------
function update_ghost_values_mdrk!(problem, scheme, eq, grid, aux, op, cache,
                                   t, dt, scaling_factor)
   ...
   if left == dirichlet
      ...
      for l=1:nd
         ...
         for n=1:nvar
            ub[n] += ubvalue[n] * scaling_factor * wg[l]
            fb[n] += fbvalue[n] * scaling_factor * wg[l]
         end
      end
      for n=1:nvar
...
function compute_cell_residual_mdrk_1!(eq::AbstractEquations{1}, grid, op,
   ...
   @inbounds for cell=1:nx # Loop over cells
      ul, ur, upl, upr, uml, umr, uppl, uppr, umml, ummr = (
         zero(get_node_vars(u1, eq, 1, cell)) for _=1:10 )
