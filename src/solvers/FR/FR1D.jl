...
function apply_tvb_limiter!(eq::AbstractEquations{1, 1}, problem, scheme, grid,
   ...
   for el_x=1:nx
      ...
      dulm = minmod(dul, ua_[el_x] - ua_[el_x-1], ua_[el_x+1] - ua_[el_x], Mdx2)
      durm = minmod(dur, ua_[el_x] - ua_[el_x-1], ua_[el_x+1] - ua_[el_x], Mdx2)
...
function modal_smoothness_indicator_new(eq::AbstractEquations{1}, t, iter,
   ...
   for i=1:nx
      ...
      un[:,1] .= 0.0
      un[:,nd+2] .= 0.0
      for ii=1:nd # get face values as average of the two cells
         for n=1:nvar
            un[n,1]    += 0.5*(u1[n,ii,i-1]*Vr[ii] + u1[n,ii,i]*Vl[ii]  )
            un[n,nd+2] += 0.5*(u1[n,ii,i]*Vr[ii]   + u1[n,ii,i+1]*Vl[ii])
         end
      end
      ...
      for n=1:n_ind_nvar
         ...
         elseif indicator_model == "model2"
            ...
            for n=1:nvar
               ...
               ind_num[n] = @views (sum((um_xg[n,:] - um[n,1:end-2]).^2)
...
function modal_smoothness_indicator_gassner_face(eq, t, iter, fcount, dt, grid,
   ...
   for i=1:nx
      ...
      ind = zeros(n_ind_nvar)
      ind_nd, ind_nd_m_1, ind_nd_m_2, ind_nd_m_3 = (zeros(n_ind_nvar), zeros(n_ind_nvar),
                                                    zeros(n_ind_nvar), zeros(n_ind_nvar))
      for n=1:n_ind_nvar
         # Last node
         ind_den = @views sum(um[n,1:end].^2)      # Gassner takes constant node
         ind_num = um[n,end]^2 # energy in last node
...
@inbounds @inline function blend_cell_residual_muscl!(cell,
   ...
   for ii=1:nd # loop over (sub)cells
      ...
      for n=1:nvar
         ...
         unph[n,2,ii] = ufr[n] + 0.5*blend.dt[1]*( (fl[n]-fr[n])
...
function Blend(eq::AbstractEquations{1}, op, grid,
   ...
   ufl, ufr, fl, fr, fn = [ zeros(nvar) for _=1:5 ]
...
function Hierarchical(eq::AbstractEquations{1}, op, grid,
   ...
   modes_cache = (OffsetArray(zeros(nvar, nd, nx+2),
                  OffsetArrays.Origin(1,1,0)) for _=1:2)
...
function write_soln!(base_name, fcount, iter, time, dt,
   ...
   if animate == true
      ...
      elseif save_time_interval > 0
         ...
         k1, k2 = ceil(t/animate_time_interval), floor(t/animate_time_interval)
