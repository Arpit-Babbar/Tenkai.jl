...
function compute_cell_residual_3!(eq::AbstractEquations{1}, grid, op, problem, scheme,
    ...
    @inbounds for cell in 1:nx # Loop over cells
        ...
        for i in Base.OneTo(nd) # Loop over solution points
            ...
            multiply_add_to_node_vars!(ft, (_1 / 12), -1.0, fpp, 8.0, fp,
                                       -8.0, fm, 1.0, fmm,
                                       eq, i)
        ...
        for i in 1:nd # Loop over solution points
            ...
            multiply_add_to_node_vars!(U, # U += (_1 / 24) * uttt
                                       (_1 / 24), uttt_node,
                                       eq, i)

            multiply_add_to_node_vars!(um,
                                       (-_1 / 6), uttt_node, # um -= (_1 / 6)*uttt
                                       eq, i)
            multiply_add_to_node_vars!(up,
                                       (_1 / 6), uttt_node, # up += (_1 / 6)*uttt
                                       eq, i)
            multiply_add_to_node_vars!(umm, # umm += 2.0*utt - (4 * _1 / 3)*uttt
                                       2.0, utt_node,
                                       (-4 * _1 / 3), uttt_node,
                                       eq, i)
            multiply_add_to_node_vars!(upp, # upp += 2.0*utt + (4 * _1 / 3)*uttt
                                       2.0, utt_node,
                                       (4 * _1 / 3), uttt_node,
                                       eq, i)
            um_node = get_node_vars(um, eq, i)
            ...
            multiply_add_to_node_vars!(F, # F += (_1 / 24) * fttt_node
                                       (_1 / 24), fttt_node,
                                       eq, i)
            F_node = get_node_vars(F, eq, i)
            for ix in 1:nd # utt[n] = -lamx * Dm * ft[n] for each n=1:nvar
        ...
        if bflux_ind == extrapolate
            ...
            multiply_add_set_node_vars!(ftl, (_1 / 12), -1.0, fppl, 8.0, fpl,
                                        -8.0, fml, 1.0, fmml,
                                        eq, 1)
            # ftr = (_1 / 12) * (-fppr + 8.0 * fpr - 8.0 * fmr + fmmr)
            multiply_add_set_node_vars!(ftr, (_1 / 12), -1.0, fppr, 8.0, fpr,
                                        -8.0, fmr, 1.0, fmmr,
                                        eq, 1)
            # fttl  = fpl - 2.0 * fl + fml
            ...
            # F = f + 0.5 * ftr + ((_1 / 6)) * fttr + ((_1 / 24)) * ftttr
            multiply_add_to_node_vars!(Fb,
                                       0.5, ftl_node,
                                       (_1 / 6), fttl_node,
                                       (_1 / 24), ftttl_node,
...
function compute_cell_residual_4!(eq::AbstractEquations{1}, grid, op, problem,
    ...
    @inbounds for cell in 1:nx # Loop over cells
        ...
        for i in Base.OneTo(nd) # Loop over solution points
            ...
            f_node = get_node_vars(f, eq, i)
            # ftt = fp - 2.0*f + fm
            multiply_add_to_node_vars!(ftt, (_1 / 12), -1.0, fpp, 16.0, fp,
                                       -30.0, f_node,
                                       16.0, fm, -1.0, fmm,
                                       eq, i)
            ftt_node = get_node_vars(ftt, eq, i)
        ...
        for i in Base.OneTo(nd) # Loop over solution points
            ...
            multiply_add_to_node_vars!(U, # U += (_1 / 24) * uttt
                                       ...
                                       eq, i)
            multiply_add_to_node_vars!(um,
                                       (_1 / 24), utttt_node, # um += (_1 / 24)*utttt
                                       eq, i)
            multiply_add_to_node_vars!(up,
                                       (_1 / 24), utttt_node, # um += (_1 / 24)*utttt
                                       eq, i)
            multiply_add_to_node_vars!(umm,
                                       (2 * _1 / 3), utttt_node, # um += (_1 / 24)*utttt
                                       eq, i)
            multiply_add_to_node_vars!(upp,
                                       (2 * _1 / 3), utttt_node, # um += (_1 / 24)*utttt
                                       eq, i)
        ...
        if bflux_ind == extrapolate
            ...
            # ftl = (_1 / 12) * (-fppl + 8.0 * fpl - 8.0 * fml + fmml)
            multiply_add_to_node_vars!(ftl, (_1 / 12), -1.0, fppl, 8.0, fpl,
                                       -8.0, fml, 1.0, fmml,
                                       eq, 1)
            # ftr = (_1 / 12) * (-fppr + 8.0 * fpr - 8.0 * fmr + fmmr)
            multiply_add_to_node_vars!(ftr, (_1 / 12), -1.0, fppr, 8.0, fpr,
                                       -8.0, fmr, 1.0, fmmr,
                                       eq, 1)
            # fttl = (_1 / 12) * (-fppl + 16.0 * fpl - 30.0 * fl + 16.0 * fml - fmml)
            multiply_add_to_node_vars!(fttl, (_1 / 12), -1.0, fppl, 16.0, fpl,
                                       -30.0, fl,
                                       16.0, fml, -1.0, fmml,
                                       eq, 1)
            # fttr = (_1 / 12) * (-fppr + 16.0 * fpr - 30.0 * fr + 16.0 * fmr - fmmr)
            multiply_add_to_node_vars!(fttr, (_1 / 12), -1.0, fppr, 16.0, fpr,
                                       -30.0, fr,
                                       16.0, fmr, -1.0, fmmr,
                                       eq, 1)
            # ftttl = 0.5 * (fppl - 2.0 * fpl + 2.0 * fml - fmml)
            ...
            # F = f + 0.5 * ft + ((_1 / 6)) * ftt + ((_1 / 24)) * fttt
            multiply_add_to_node_vars!(Fb, # F += (_1 / 24) * fttt_node
                                       0.5, ftl_node,
                                       (_1 / 6), fttl_node,
                                       (_1 / 24), ftttl_node,
                                       (_1 / 120), fttttl_node,
