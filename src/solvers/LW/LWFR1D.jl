   ...
   @inbounds for cell=1:nx # Loop over cells
      ...
      for i=1:nd # Loop over solution points
         ...
         multiply_add_to_node_vars!(upp, # upp += 2.0*utt + 4.0/3.0*uttt
                                     ...
                                     4.0/3.0, uttt_node,
                                     eq, i)
         um_node = get_node_vars(um, eq, i); fm = flux(x_, um_node, eq)
         up_node = get_node_vars(up, eq, i); fp = flux(x_, up_node, eq)
         umm_node = get_node_vars(umm, eq, i); fmm = flux(x_, umm_node, eq)
         upp_node = get_node_vars(upp, eq, i); fpp = flux(x_, upp_node, eq)
